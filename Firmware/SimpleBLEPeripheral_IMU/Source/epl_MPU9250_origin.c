...
**====================================================================================================*/
/*====================================================================================================*/
void MPU9250_getData( uint16 *dataIMU ) //test failed ,wrong value of Magnetometer
{
    uint8 tmpRead[22] = {0};
   ...
   // WAIT_US(1000);
    ...
    ReadRegs(MPUREG_ACCEL_XOUT_H, tmpRead, 22);
 
    //dataIMU[0] = ((uint16)tmpRead[6] << 8 |  tmpRead[7]);    // Temp
    dataIMU[0] = ((uint16)tmpRead[0] << 8 |  tmpRead[1]);  // Acc.X
    dataIMU[1] = ((uint16)tmpRead[2] << 8 |  tmpRead[3]);  // Acc.Y
    dataIMU[2] = ((uint16)tmpRead[4] << 8 |  tmpRead[5]);  // Acc.Z
    dataIMU[3] = ((uint16)tmpRead[8] << 8 |  tmpRead[9]);  // Gyr.X
    dataIMU[4] = ((uint16)tmpRead[10] << 8 |  tmpRead[11]);  // Gyr.Y
    dataIMU[5] = ((uint16)tmpRead[12] << 8 |  tmpRead[13]);  // Gyr.Z

//    if(!(tmpRead[14] & AK8963_STATUS_DRDY) || (tmpRead[14] & AK8963_STATUS_DOR) || (tmpRead[21] & AK8963_STATUS_HOFL))
//        return;

    dataIMU[6] = ((uint16)tmpRead[15] << 8 |  tmpRead[14]);  // Mag.X
    dataIMU[7] = ((uint16)tmpRead[17] << 8 |  tmpRead[16]);  // Mag.Y
    dataIMU[8] = ((uint16)tmpRead[19] << 8 |  tmpRead[18]);  // Mag.Z
...
bool mpu9250_init(uint8 dlpf_cfg, uint8 a_dlpf_cfg){
    ...
    /*AK8963_ASA[0] = (uint16)(tmpRead[0]) + 128;
    AK8963_ASA[1] = (uint16)(tmpRead[1]) + 128;
    AK8963_ASA[2] = (uint16)(tmpRead[2]) + 128;*/
...
{  
  uint16 gyrosensitivity, accelsensitivity, i=0;
  int16 ax, ay, az, gx, gy, gz;
  int32 gyro_zero[3]={0}, acc_zero[3]={0};
  int count = 2000;
//  char buf[100];
  
  gyrosensitivity = get_gyro_divider();
  accelsensitivity = get_acc_divider();
...
//  sprintf(buf,"Acc and Gryo calibration done\r\n");
//  uartWriteString(buf);
}
...
#pragma optimize=none
void read_Mag(uint16* Magnetometer){
    uint8 response[7];
    int i;
